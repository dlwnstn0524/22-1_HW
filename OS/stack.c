...
void push_stack(char *string)
{
	...
	entry->string = (char*)malloc(sizeof(char)*strlen(string)+1);
...
int pop_stack(char *buffer)
{
	...
	strcpy(buffer,list_last_entry(&stack, struct entry, list)->string);
	struct entry *i = list_last_entry(&stack, struct entry, list);
	list_del(&list_last_entry(&stack, struct entry, list)->list);
	free(i->string);
	free(i);
	return 0;
}
...
void dump_stack(void)
{
	/* TODO: Implement this function */
	struct entry *a;
	list_for_each_entry(a, &stack, list){
		fprintf(stderr,"%s\n", a->string);
	}
}
